...
(function () {
    ...
    GM_addStyle(`
        ...
        .trm-review-event.trm-flagged-outcome { border-left-color: #ff6b6b; background: #3b2424; }
        .trm-review-outcome { color: #ff8d8d; }
    ...
    async function poll(manual = false) {
        ...
        try {
            const now = unixNow(); const attacks = await getFactionAttacks(tornKey, now - LOOKBACK_SECONDS, now); removeTargetsAlreadyHit(attacks); await recordOutgoingReviewEvents(attacks, tornKey, now);
            if (!runtimeShouldRun()) return;
    ...
    function getAttackResult(attack) { return String(attack?.result ?? attack?.outcome ?? attack?.attack_result ?? '').trim(); }
    function getFlaggedOutcome(attack) { const result = getAttackResult(attack).toLowerCase().replace(/[_-]+/g, ' ').replace(/\s+/g, ' ').trim(); if (result === 'lost' || result === 'loss') return 'Lost'; if (result === 'escape' || result === 'escaped') return 'Escape'; return ''; }
    function isOutgoingChainFailure(attack) { const attackerFaction = getFactionId(attack.attacker); const defenderFaction = getFactionId(attack.defender); return attackerFaction === factionId && defenderFaction !== factionId && Boolean(getPlayerId(attack.defender)) && Boolean(getFlaggedOutcome(attack)); }
    ...
    async function recordOutgoingReviewEvents(attacks, apiKey, now = unixNow()) {
        ...
        const unseen = attacks.filter(attack => { const attackId = String(attack?.id ?? attack?.attack_id ?? ''); return attackId && !seen.has(attackId) && getAttackEnded(attack) >= enabledAt; });
        if (!unseen.length) return;
        const observedAt = unixNow();
        const flaggedAdditions = unseen.filter(isOutgoingChainFailure).map(attack => {
            const attackerId = getPlayerId(attack.attacker); const defenderId = getPlayerId(attack.defender); const ended = getAttackEnded(attack); const opponentFactionId = getFactionId(attack.defender);
            return { attackId: String(attack.id ?? attack.attack_id), attackEnded: ended, observedAt, observedDelaySeconds: Math.max(0, observedAt - ended), attackerId, attackerName: getPlayerName(attack.attacker, attackerId), defenderId, defenderName: getPlayerName(attack.defender, defenderId), opponentFactionId, opponentFactionName: getFactionName(attack.defender) || (opponentFactionId ? `Faction ${opponentFactionId}` : ''), reviewKind: 'chain_failure', outcome: getFlaggedOutcome(attack), observedStatus: '', observedOnline: false };
        });
        const baseLog = [...log, ...flaggedAdditions];
        if (flaggedAdditions.length) saveOutgoingReviewLog(baseLog);
        const warHits = unseen.filter(isOutgoingRankedWarAttack);
        if (!warHits.length) return;
        const opponent = resolveReviewOpponent(warHits, now); if (!opponent) return;
        const qualifying = warHits.filter(attack => getFactionId(attack.defender) === opponent.id); if (!qualifying.length) return;
        let roster;
        ...
        if (!runtimeShouldRun()) return;
        const statusObservedAt = unixNow();
        const additions = qualifying.map(attack => { const attackerId = getPlayerId(attack.attacker); const defenderId = getPlayerId(attack.defender); const status = normalizeOnlineStatus(roster.get(defenderId)); const ended = getAttackEnded(attack); return { attackId: String(attack.id ?? attack.attack_id), attackEnded: ended, observedAt: statusObservedAt, observedDelaySeconds: Math.max(0, statusObservedAt - ended), attackerId, attackerName: getPlayerName(attack.attacker, attackerId), defenderId, defenderName: getPlayerName(attack.defender, defenderId), opponentFactionId: opponent.id, opponentFactionName: opponent.name, reviewKind: 'war_status', outcome: getAttackResult(attack), observedStatus: status, observedOnline: status === 'Online' }; });
        saveOutgoingReviewLog([...baseLog, ...additions]);
    }

    function isFlaggedReviewItem(item) { return String(item?.reviewKind || '') === 'chain_failure'; }
    function updateOutgoingReviewButton() { const button = document.getElementById('trm-review'); if (!button) return; const log = getOutgoingReviewLog(); const flaggedCount = log.filter(isFlaggedReviewItem).length; const onlineCount = log.filter(item => !isFlaggedReviewItem(item) && item.observedOnline).length; button.classList.toggle('trm-active', showingOutgoingReview); button.textContent = flaggedCount ? `LOG !${flaggedCount}` : onlineCount ? `LOG ${onlineCount}` : 'LOG'; button.title = `${outgoingReviewEnabled() ? 'Outgoing attack review' : 'Outgoing attack logging is disabled'}${flaggedCount ? ` • ${flaggedCount} chain losses or escapes` : ''}${onlineCount ? ` • ${onlineCount} war targets observed online` : ''}`; }
    function reviewCsvCell(value) { const text = String(value ?? ''); return /[",\r\n]/.test(text) ? `"${text.replaceAll('"', '""')}"` : text; }
    function exportOutgoingReviewCsv(items, attackerName = '') { const headers = ['attack_id','attack_time','review_kind','outcome','our_attacker_id','our_attacker_name','enemy_defender_id','enemy_defender_name','enemy_faction_id','enemy_faction_name','observed_status','observed_online','status_checked_at','observation_delay_seconds']; const rows = items.map(item => { const flagged = isFlaggedReviewItem(item); return [item.attackId,new Date(Number(item.attackEnded || 0) * 1000).toISOString(),flagged ? 'chain_failure' : 'war_status',item.outcome || '',item.attackerId,item.attackerName,item.defenderId,item.defenderName,item.opponentFactionId,item.opponentFactionName,item.observedStatus || '',flagged ? '' : item.observedOnline ? 'yes' : 'no',flagged ? '' : new Date(Number(item.observedAt || 0) * 1000).toISOString(),flagged ? '' : item.observedDelaySeconds]; }); const csv = [headers,...rows].map(row => row.map(reviewCsvCell).join(',')).join('\r\n'); const safeName = String(attackerName || 'all').replace(/[^a-z0-9_-]+/gi,'-').replace(/^-+|-+$/g,'') || 'all'; const blob = new Blob([csv],{type:'text/csv;charset=utf-8'}); const url = URL.createObjectURL(blob); const link = document.createElement('a'); link.href=url; link.download=`torn-outgoing-attack-review-${safeName}-${new Date().toISOString().slice(0,10)}.csv`; document.body.appendChild(link); link.click(); link.remove(); setTimeout(()=>URL.revokeObjectURL(url),1000); }
    function reviewSummaryDetail(row) { const parts = []; if (row.lost) parts.push(`${row.lost} Lost`); if (row.escaped) parts.push(`${row.escaped} Escape`); if (row.warHits) parts.push(`${row.online} observed Online`, `${row.idle} Idle`, `${row.offline} Offline${row.unknown ? `, ${row.unknown} Unknown` : ''}`); return parts.join(' • ') || 'No review events'; }
    function reviewSummaryMetric(row) { const lines = []; if (row.flagged) lines.push(`${row.flagged} flagged`); if (row.warHits) lines.push(`${Math.round(row.online / row.warHits * 100)}% online`); lines.push(`${row.total} attacks`); return lines.join('<br>'); }
    function renderReviewEvent(item) {
        const attacker = `<a href="https://www.torn.com/profiles.php?XID=${encodeURIComponent(item.attackerId)}" target="_blank" rel="noopener noreferrer">${escapeHtml(item.attackerName)} [${item.attackerId}]</a>`;
        const defender = `<a href="https://www.torn.com/profiles.php?XID=${encodeURIComponent(item.defenderId)}" target="_blank" rel="noopener noreferrer">${escapeHtml(item.defenderName)} [${item.defenderId}]</a>`;
        const attackTime = new Date(Number(item.attackEnded || 0) * 1000).toLocaleString();
        const faction = item.opponentFactionName || (item.opponentFactionId ? `Faction ${item.opponentFactionId}` : 'No faction');
        if (isFlaggedReviewItem(item)) return `<div class="trm-review-event trm-flagged-outcome"><strong class="trm-review-outcome">${escapeHtml(String(item.outcome || 'Failed').toUpperCase())}</strong> • ${attacker} attacked ${defender}<br><span class="trm-review-event-time">${escapeHtml(attackTime)} • ${escapeHtml(faction)} • attack ${escapeHtml(item.attackId)}</span></div>`;
        const delay = Number(item.observedDelaySeconds || 0); const delayText = delay < 60 ? `${delay}s` : `${Math.floor(delay / 60)}m ${delay % 60}s`;
        return `<div class="trm-review-event ${item.observedOnline ? 'trm-observed-online' : ''}">${attacker} attacked ${defender}<br>Observed: <strong class="${item.observedOnline ? 'trm-online' : 'trm-offline'}">${escapeHtml(item.observedStatus || 'Unknown')}</strong> • checked ${delayText} after attack<br><span class="trm-review-event-time">${escapeHtml(attackTime)} • ${escapeHtml(faction)} • attack ${escapeHtml(item.attackId)}</span></div>`;
    }
    function renderOutgoingReview(body) {
        const log = getOutgoingReviewLog().sort((a,b) => Number(b.attackEnded || 0) - Number(a.attackEnded || 0));
        const flaggedCount = log.filter(isFlaggedReviewItem).length;
        const onlineCount = log.filter(item => item.observedOnline).length;
        const byAttacker = new Map();
        for (const item of log) { const id = Number(item.attackerId || 0); if (!byAttacker.has(id)) byAttacker.set(id, { id, name: String(item.attackerName || `Player ${id}`), total: 0, flagged: 0, lost: 0, escaped: 0, warHits: 0, online: 0, idle: 0, offline: 0, unknown: 0 }); const row = byAttacker.get(id); row.total += 1; if (isFlaggedReviewItem(item)) { row.flagged += 1; if (String(item.outcome || '').toLowerCase() === 'lost') row.lost += 1; else row.escaped += 1; continue; } row.warHits += 1; const status = String(item.observedStatus || 'Unknown').toLowerCase(); if (status === 'online') row.online += 1; else if (status === 'idle') row.idle += 1; else if (status === 'offline') row.offline += 1; else row.unknown += 1; }
        const summaries = [...byAttacker.values()].sort((a,b) => b.flagged - a.flagged || b.online - a.online || b.total - a.total || a.name.localeCompare(b.name));
        ...
        const summaryHtml = summaries.map(row => `<button type="button" class="trm-review-person ${selected?.id === row.id ? 'trm-selected' : ''}" data-review-attacker="${row.id}"><span><strong>${escapeHtml(row.name)}${row.id ? ` [${row.id}]` : ''}</strong><br><span>${escapeHtml(reviewSummaryDetail(row))}</span></span><span>${reviewSummaryMetric(row)}</span></button>`).join('');
        const eventsHtml = visibleLog.map(renderReviewEvent).join('');
        const title = selected ? `${selected.name}'s outgoing attacks` : 'Outgoing attack review';
        const note = selected ? `Showing all ${visibleLog.length} recorded review events by this member, including chain-wide losses or escapes and ranked-war status samples.` : `${outgoingReviewEnabled() ? 'Logging is enabled.' : 'Logging is disabled in Settings.'} ${flaggedCount} chain-wide losses or escapes are flagged; these use only the existing attacks response and do not trigger status lookups. ${onlineCount} successful ranked-war hits were observed Online. Click a member to see every recorded event.`;
        body.innerHTML = `<div class="trm-review-toolbar"><div class="trm-review-title">${escapeHtml(title)}</div><button class="trm-btn trm-review-small-button" id="trm-review-back">${selected ? 'All attackers' : 'Back'}</button></div><div class="trm-review-note">${escapeHtml(note)}</div>${!selected && summaryHtml ? `<div class="trm-review-summary">${summaryHtml}</div>` : !log.length ? '<div class="trm-empty">No outgoing attack review events have been recorded yet.</div>' : ''}${eventsHtml ? `<div class="trm-review-events">${eventsHtml}</div><div class="trm-settings-actions"><button class="trm-btn" id="trm-review-export">Export ${selected ? 'this member' : 'all'} CSV</button><button class="trm-btn" id="trm-review-clear">Clear review log</button></div>` : ''}`;
        ...
        document.getElementById('trm-review-clear')?.addEventListener('click', () => { if (!confirm('Clear the outgoing attack review log?')) return; saveOutgoingReviewLog([]); if (outgoingReviewEnabled()) GM_setValue(KEYS.outgoingReviewEnabledAt, unixNow()); render(); });
    }
